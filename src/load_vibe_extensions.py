...
if not VIBE_AVAILABLE:
    ...
    vibe_path = subprocess.run(["which", "vibe"], capture_output=True, text=True).stdout.strip()

    if vibe_path:
        # Run this script again with the correct Python
        python_path = (
            subprocess.run(["readlink", "-f", vibe_path], capture_output=True, text=True)
            .stdout.strip()
            .replace("/bin/vibe", "/bin/python3")
        )
...
from vibe.cli.entrypoint import main  # noqa: E402
